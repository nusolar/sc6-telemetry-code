...
(function() {
  ...
  populateCar = function() {
    ...
    return $('#bbox_svg').load('css/bbox.svg', drawCar);
  };
  ...
  populateSend = function() {
    ...
    for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
      g = _ref[i];
      $('#pkt_name')[0].options[i] = new Option(g[0], g[1]);
    }
    return $('#pkt_name')[0].onchange = drawSend;
  };

  redraw = function() {
    centerTabs();
    return [drawCar, drawPlots, drawStrategy, (function() {}), drawSend][this.page]();
  };
  ...
  centerTabs = function() {
    ...
    return $('#t1').css('margin-left', ($('#nav').width() - margin) / 2);
  };

  drawCar = function() {
    var maxH, ns, setText, x, _i, _results;
    if (!($('#driver_svg').children().size() && $('#bbox_svg').children().size())) {
      return true;
    }
  ...
  drawPlots = function() {
    ...
    $('#plot_holder').height(height < maxH ? height : maxH);
  ...
  drawStrategy = function() {
    ...
    $('#strategy_holder').height(height < maxH ? height : maxH);
    return $.plot('#strategy_holder', [[0, 1], [1, 1.2]]);
  };

  drawSend = function() {
    return $('#pkt_addr').val($('#pkt_name').val());
  };
